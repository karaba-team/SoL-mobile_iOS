...
//                newPos = self.boundLayerPos(aNewPosition: newPos)
                ...
                if isThere3Obj(){
                    if isChildBesideBoss(child: titikBeratAnak){
                        ...
                        if let view = gameVC.skView {
                            ...
                            if let scene = SKScene(fileNamed: "Chapter15Scene") as? Chapter15Scene {
                                // Set the scale mode to scale to fit the window
                                
                                let transition = SKTransition.fade(with: .white, duration: 2.5)
                                scene.scaleMode = .aspectFill
                                scene.gameVC = gameVC
                                scene.gameVC.chapter15Scene = scene as Chapter15Scene
                                // Present the scene
                                view.presentScene(scene, transition: transition)
                                gameVC.reloadCollection()
                                gameVC.changeScene(sceneNo: 8)
                            }
    ...
    func checkChildArea(child: CGPoint, boss: [CGPoint]) -> String{
        if child.x >= boss[0].x && child.x >= boss[3].x && child.x <= boss[1].x && child.x <= boss[2].x{
            if child.y < boss[2].y && child.y < boss[3].y && child.y > boss[2].y - 5 && child.y > boss[3].y - 5{
                return "bawah"
            }
        }
        return ""

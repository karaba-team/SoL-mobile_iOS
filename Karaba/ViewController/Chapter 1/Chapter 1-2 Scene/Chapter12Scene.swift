...
//                newPos = self.boundLayerPos(aNewPosition: newPos)
    ...
    @objc func handlePinchFrom(_ sender: UIPinchGestureRecognizer) {
        ...
        if sender.state == .ended {
            ...
            if isThere3Obj() && isThereABoss(points: currentFrameDots){
                if let scene = Chapter13Scene(fileNamed: "Chapter13Scene"){
                    gameVC.changeScene(sceneNo: 6)
                    let transition = SKTransition.fade(with: .white, duration: 2.5)
                    scene.scaleMode = .aspectFill
                    scene.gameVC = gameVC
                    scene.gameVC.chapter13Scene = scene as Chapter13Scene
